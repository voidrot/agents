...
# Export the lockfile to requirements.txt (hashes are included by default)
...
uv resolves, downloads, and installs packages concurrently by default. The
`uv pip install` command does not expose a `--jobs` option; avoid documenting a
manual parallelism setting and let uv manage concurrency.
