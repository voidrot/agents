...
description: Use when working with devenv.nix, languages.*/services.* options, git-hooks,
  devenv shell/up/build, or devenv development environments.
metadata:
  version: 3.0.0
  license-status: UNCONFIRMED
---
Patterns for devenv configuration — language setup, services, tooling (git-hooks, scripts, processes),
outputs, and profiles. For Nix language syntax, flake structure, or overlays, see
nix-best-practices instead; this skill covers only what is devenv-specific.
...
- nix-best-practices — Nix language syntax, flake structure and inputs, overlays;
  devenv builds on all of it but doesn't re-teach it.
