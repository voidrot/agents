...
1. **State the discrepancy and boundary.** Record the desired observable behavior, actual observable behavior, affected inputs, environment, timing, and impact. Separate facts from interpretations. Read the relevant contract, callers, tests, recent changes, configuration, and operational artifacts before modifying code. Preserve secrets: use environment-variable names and redact values, tokens, headers, and sensitive payload fields in retained evidence.
