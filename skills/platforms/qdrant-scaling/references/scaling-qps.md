...
- Use fewer, larger segments (`default_segment_number: 2`) [Maximizing throughput](https://qdrant.tech/documentation/)
- Enable quantization with `always_ram=true` to reduce disk IO [Quantization](https://qdrant.tech/documentation/)
- Use batch search API to amortize overhead [Batch search](https://qdrant.tech/documentation/)

## Minimize impact of Update Workloads

- Configure update throughput control (v1.17+) to prevent unoptimized searches degrading reads [Low latency search](https://qdrant.tech/documentation/)
- Set `optimizer_cpu_budget` to limit indexing CPUs (e.g. `2` on an 8-CPU node reserves 6 for queries)
- Configure delayed read fan-out (v1.17+) for tail latency [Delayed fan-outs](https://qdrant.tech/documentation/)
...
- Use `replication_factor: 2+` and route reads to replicas [Distributed deployment](https://qdrant.tech/documentation/)

See also [Horizontal Scaling](scaling-data-volume/horizontal-scaling.md) for general horizontal scaling guidance.
...
- In case of quantized vectors, prefer global rescoring over per-segment rescoring to reduce disk reads. Example in the [tutorial](https://qdrant.tech/documentation/)
- Configure higher number of search threads to parallelize disk reads. Default is `cpu_count - 1`, which is optimal for RAM-based search but may be too low for disk-based search. See [configuration reference](https://qdrant.tech/documentation/)
