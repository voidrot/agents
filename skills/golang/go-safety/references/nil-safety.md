...
func IsZero(value T) bool { // T is constrained by comparable
    var zero T
    return value == zero // works, but "zero" for *Foo is nil, for int is 0
}

// ✓ Better — be explicit about what "empty" means
func IsNil(value T) bool { // T is constrained to pointer-like types
    return value == nil
}
```
...
```go
// ✗ Does not compile
func Check(value T) bool { // T is unconstrained
    return value == nil // compile error: cannot compare T with nil
}

// ✓ Good — use reflect or constrain to pointer types
func IsNilPtr(value *T) bool {
    return value == nil
}
```
...
→ See `go-error-handling` skill for nil error comparison pitfalls.
