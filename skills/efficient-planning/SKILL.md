...
- For a bug fix, include a regression test or equally durable reproduction that would fail for the reported defect. Use the `efficient-testing` skill to scope and execute focused regression coverage. If that is infeasible, state why and retain the strongest practical evidence.
