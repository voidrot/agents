...
description: Use when writing, reviewing, or debugging Terraform/OpenTofu modules,
  tests, CI, scans, or state ops - diagnoses failure mode (identity churn, secrets,
  blast radius, CI drift, state corruption) with version-aware guards.
license: Apache-2.0
metadata:
...
Apply this self-contained terraform-ls layer directly. It includes position anchoring, a degradation gate, and a disclosure format, so no companion plugin is required.
