...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Use secrets
        run: |
          # Secrets injected as env vars — never print them to logs
          # Use $DB_USERNAME, $DB_PASSWORD, $API_KEY
```
...
**Further guidance:** Consult the current HashiCorp Vault documentation for deployment-specific setup.
...
**Further guidance:** Consult current GitHub Actions documentation for repository and environment secret settings.
...
- Current official GitLab CI documentation - For GitLab CI integration
