...
3. DSNs are routing/configuration identifiers, not secret credentials; use placeholders and avoid copying production DSNs into examples, logs, or user-visible evidence to minimize unnecessary configuration disclosure. Never put tokens, credentials, PII, raw payloads, request/response bodies, headers, or secrets in code, commands, examples, logs, or evidence.
...
4. **Filter before send.** Make call sites safe first. If the selected runtime's current docs support a pre-send metric hook, use it to enforce the metric contract and volume/cardinality policy: drop known noisy names/environments and reject attributes outside the bounded allowlist. This is not default privacy scrubbing; sensitive data must be excluded at the source and is subject to expected Sentry server-side scrubbing rules. Do not use filtering to make per-request/user emission acceptable. Keep transformations deterministic and test them locally where feasible.
