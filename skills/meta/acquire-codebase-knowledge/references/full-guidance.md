...
Use the scan output to answer questions for each of the seven templates. Load [`inquiry-checkpoints.md`](inquiry-checkpoints.md) for the full per-template question list.

If the stack is ambiguous (multiple manifest files, unfamiliar file types, no `package.json`), load [`stack-detection.md`](stack-detection.md).
...
1. Validate each doc against `inquiry-checkpoints.md`.
...
| [`inquiry-checkpoints.md`](inquiry-checkpoints.md) | Phase 2 — load for per-template investigation questions |
| [`stack-detection.md`](stack-detection.md) | Phase 2 — only if stack is ambiguous |
