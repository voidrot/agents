...
- User is asking about ECC configuration (send them to the project's
  configuration guidance instead)
- User wants a skill inventory (use the project's component inventory workflow
  instead of prompt optimization)
...
2. Detect tech stack from project files:
   ...
   - `Cargo.toml` → Rust
   - `build.gradle` / `pom.xml` → Java / Kotlin (then check for Quarkus or Spring Boot markers in the build file)
   - `Package.swift` → Swift
   - `Gemfile` → Ruby
   - `composer.json` → PHP
   - `*.csproj` / `*.sln` → .NET
...
|-----------|------------------|
| ECC configuration guidance | User hasn't set up ECC yet |
| Component inventory workflow | Audit which components are installed instead of relying on a hardcoded catalog |
| Search-first workflow | Research phase in optimized prompts |
| Blueprint planning skill | EPIC-scope optimized prompts; invoke as a skill, not a command |
| Strategic compaction workflow | Long session context management |
