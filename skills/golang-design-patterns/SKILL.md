...
description: Idiomatic Golang design patterns — functional options, constructors,
  error flow and cascading, resource management and lifecycle, graceful shutdown,
  resilience, architecture, dependency injection, data handling, streaming, and more.
  Apply when explicitly choosing between architectural patterns, implementing functional
  options, designing constructor APIs, setting up graceful shutdown, applying resilience
  patterns, or asking which idiomatic Go pattern fits a specific problem.
license: MIT
metadata:
  author: samber
  version: 1.2.0
...
> **Community default.** A company skill that explicitly supersedes `golang-design-patterns` skill takes precedence.
...
12. **Use `strings.Builder`** for concatenation in loops → see `golang-code-style`
...
17. **Use `crypto/rand`** for keys/tokens — `math/rand` is predictable → see `golang-security`
...
- **Respect 12-factor app** principles — → see `golang-project-layout`
...
- → See `golang-data-structures` skill for data structure selection, internals, and container/ packages
- → See `golang-error-handling` skill for error wrapping, sentinel errors, and the single handling rule
