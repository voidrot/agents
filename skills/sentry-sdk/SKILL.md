...
2. Never print, commit, or put real DSNs, auth tokens, source-map/debug-symbol credentials, or PII in examples, output, or test fixtures. Use placeholders such as `<SENTRY_DSN>`.
...
4. **Install and initialize.** Use the platform's current official installation and initialization path. Initialize at the earliest documented runtime entry point, once per runtime, before code to be observed. Load the DSN from approved runtime configuration, never a checked-in literal.
...
6. **Make events usable.** Set release/environment/dist or equivalent only from the deployed build identity; keep artifact identifiers and paths aligned with that build. Configure filtering and scrubbers before sending production data. See [safety/data](references/common-safety-and-data.md).
