...
The underlying command that starts the server (`gopls mcp`) is harness-agnostic. Configure an MCP-capable host to launch it over stdio using that host's documented connection settings.
...
A host-provided editor-style LSP integration is distinct from the MCP server above. Use it when the current host supports it.

**Enabling it:**

1. Install `gopls` (`go install golang.org/x/tools/gopls@latest`).
2. Configure the current host's LSP integration to use `gopls`, if available.
...
Either mode — MCP or native `LSP` — only ever reasons about code that is present and resolvable in the local build: the workspace plus every dependency exactly as pinned in `go.sum`, including `replace` directives. For anything outside that boundary, → See `golang-pkg-go-dev` skill (`godig`).
