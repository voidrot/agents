...
description: Provides a guide for setting up Golang project layouts and workspaces.
  Use when starting a new Go project, organizing an existing codebase, setting up
  a monorepo with multiple packages, creating CLI tools with multiple main packages,
  deciding between cmd/internal/pkg directory conventions, or discussing package restructuring,
  package splits, or module splits.
license: MIT
metadata:
  author: samber
  version: 1.4.0
...
Packages MUST be lowercase, singular, and match their directory name. → See `golang-naming` skill for complete package naming conventions and examples.
...
- [ ] Document any project-specific Go conventions in the repository's existing agent instruction file when they are stable and genuinely required

## Related Skills

Use `golang-cli` for CLI structure and Cobra/Viper patterns, `golang-dependency-injection` for DI comparison and wiring, `golang-lint` for golangci-lint configuration, `golang-continuous-integration` for CI/CD, `golang-design-patterns` for architecture, and `golang-refactoring` for staged structural changes.
