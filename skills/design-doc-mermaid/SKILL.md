...
description: Create Mermaid diagrams (flowchart, sequence, class, ER, state, C4, architecture)
  from text or source code. Default for GitHub wiki. Use when asked to create a diagram,
  generate mermaid, document architecture, or convert code to diagram. PlantUML is
...
2. **Search authoritative docs** - Use an available web or documentation search tool for syntax questions
3. **Search recent reports** - Check current issue trackers and community discussions
4. **Get a second opinion** - Ask an available independent model or subagent
5. **General search** - Use any available web search tool as a fallback
...
8. On success → Add `!API Sequence` to markdown
...
│       ├── resilient-workflow.md         # Validation-first generation workflow
│       ├── wiki-ticket-and-github.md     # GitHub wiki, WikiTicket, Confluence
...
| "design document", "full docs" | a matching template in `assets/` + diagram guides |
...
4. Use `scripts/extract_mermaid.py document.md --validate` to check your work

**Need to document code?** Follow this:
1. Identify your framework → Load relevant the matching example directory under `examples/`
...
**Source attribution:** historical the original source distribution
