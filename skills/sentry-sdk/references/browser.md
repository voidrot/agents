...
2. Initialize once before application rendering; add supported error, navigation, fetch/XHR, replay, or profiling instrumentation only for an agreed need. Deliberately configure sampling and propagation targets. Expect sensitive-data scrubbing through Sentry server-side rules; use client-side privacy filtering only when the user explicitly requests it, as optional defense in depth.
