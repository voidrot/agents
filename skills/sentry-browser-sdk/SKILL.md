...
description: Full Sentry SDK setup for browser JavaScript. Use when asked to "add
  Sentry to a website", "install @sentry/browser", or configure error monitoring,
  tracing, session replay, or logging for vanilla JavaScript, jQuery, static sites,
  or WordPress.
license: Apache-2.0
metadata:
  category: sdk-setup
...
---
> All Skills > sentry-sdk-setup > Browser SDK
...
| `@sveltejs/kit` | Follow current official Sentry Svelte documentation — **do not proceed here** |
...
| `express` / `fastify` / `@nestjs/core` | This is a Node.js server — use `sentry-node-sdk`; for NestJS, follow current official Sentry NestJS documentation |
...
| Tracing | `references/tracing.md` | Page load / API call tracing |
| Session Replay | `references/session-replay.md` | User-facing app |
| Logging | `references/logging.md` | Structured log search; npm or CDN logs bundle (not Loader Script) |
| Profiling | `references/profiling.md` | Performance-critical, Chromium-only |
| User Feedback | `references/user-feedback.md` | Capture user reports after errors |
...
| Ruby (`Gemfile`) | current official Sentry Ruby documentation |
| PHP (`composer.json`) | current official Sentry PHP documentation |
| .NET (`*.csproj`, `*.sln`) | current official Sentry .NET documentation |
...
| NestJS (`@nestjs/core`) | current official Sentry NestJS documentation |
