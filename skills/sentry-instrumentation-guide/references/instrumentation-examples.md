...
> and its relevant bundled reference material. When that is unavailable, consult current official Sentry documentation; this guide intentionally does not duplicate version-specific setup.
...
the `sentry-python-sdk` skill or current official Sentry documentation for the full custom-span API):
