...
1. **Establish scope and preserve the boundary.**
   ...
   - Classify the work: read [workflow guidance](references/workflows.md) for a workflow change or failure, and [custom-action guidance](references/actions.md) for an `action.yml`/`action.yaml` change. Read both when a workflow calls an action being changed. When the request explicitly involves CodeQL setup/configuration, SARIF, or CodeQL build/analysis troubleshooting, also read the conditional [CodeQL reference](references/codeql.md).
