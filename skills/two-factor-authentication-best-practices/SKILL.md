...
description: Configure TOTP authenticator apps, send OTP codes via email/SMS, manage
  backup codes, handle trusted devices, and implement 2FA sign-in flows using Better
  Auth's twoFactor plugin. Use when users need MFA, multi-factor authentication, authenticator
...
3. Run `npx auth@latest migrate apply` (built-in adapter) or generate the schema and apply it with your ORM's migration tooling (for example, `drizzle-kit push` in development or `prisma migrate dev`)
4. Verify: check that the user table has `twoFactorEnabled` and that the `twoFactor` table has the `secret` column
...
For credential accounts, requires password verification. Returns a TOTP URI (for a QR code) and backup codes; enabling OTP instead of TOTP returns no TOTP URI.
...
Credential accounts require password confirmation for 2FA management. With `allowPasswordless: true`, users without a credential account can enable and manage 2FA without a password.
