...
description: Full Sentry SDK setup for Node.js, Bun, and Deno. Use when asked to "add
  Sentry to Node.js", "add Sentry to Bun", "add Sentry to Deno", "install @sentry/node",
  "@sentry/bun", or "@sentry/deno", or configure error monitoring, tracing, logging,
  profiling, metrics, crons, or AI monitoring for server-side JavaScript/TypeScript
  runtimes.
license: Apache-2.0
metadata:
  category: sdk-setup
...
---
> All Skills > sentry-sdk-setup > Node.js / Bun / Deno SDK
...
> **NestJS?** Use current official Sentry NestJS documentation instead — it uses `@sentry/nestjs` with NestJS-native decorators and filters.
> **Next.js?** Use sentry-nextjs-sdk instead — it handles the three-runtime architecture (browser, server, edge).

> **Note:** SDK versions below reflect current Sentry docs at time of writing (`@sentry/node` ≥10.42.0, `@sentry/bun` ≥10.42.0, `@sentry/deno` ≥10.42.0). The OTLP and runtime-metrics examples require `@sentry/node`/`@sentry/bun` ≥10.47.0.
...
> **Follow the current official Sentry NestJS documentation instead.**
...
| NestJS | Follow current official Sentry NestJS documentation | Framework-specific guidance | — |
...
| AI Monitoring | Load `sentry-instrument` skill | OpenAI, Anthropic, LangChain, Vercel AI, Google GenAI |

For each feature: read the reference file, follow its steps exactly, and verify before moving on.

### Runtime Metrics

**Requires `@sentry/node` or `@sentry/bun` ≥10.47.0.** Automatically collect Node.js and Bun runtime health metrics (memory, CPU utilization, event loop delay/utilization, uptime) at a configurable interval. Metrics appear in Sentry's Metrics product under the `node.runtime.*` / `bun.runtime.*` namespace.
...
**Requires `@sentry/node-core` ≥10.47.0.** For OTel-first projects using `@sentry/node-core/light`. Import: `import { otlpIntegration } from '@sentry/node-core/light/otlp'`.
...
Upload needs a `SENTRY_AUTH_TOKEN` (a build-time secret). For creating the token and wiring it into CI, see `sentry-fix-stack-traces`.
...
|----------|-------------|-----|
| NestJS (`@nestjs/core` in `package.json`) | current official Sentry NestJS documentation | Uses `@sentry/nestjs` with NestJS-native decorators, filters, and GraphQL support |
...
| Svelte/SvelteKit | current official Sentry Svelte documentation |
...
| Ruby backend (`Gemfile`) | current official Sentry Ruby documentation |
...
| NestJS: errors not captured | Wrong SDK or missing filter | Use current official Sentry NestJS documentation — NestJS needs `@sentry/nestjs`, not `@sentry/node` |
