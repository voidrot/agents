...
class DiagramResult:
    ...
    search_recommendation: str | None = None
...
class ResilientDiagramGenerator:
    ...
    # Precompiled, immutable patterns for detecting diagram type from the first line.
    DIAGRAM_TYPE_PATTERNS: ClassVar = MappingProxyType({
        DiagramType.FLOWCHART: (
            re.compile(r'^flowchart\s+(TB|TD|BT|RL|LR)', re.IGNORECASE),
            re.compile(r'^graph\s+(TB|TD|BT|RL|LR)', re.IGNORECASE),
        ),
        DiagramType.SEQUENCE: (re.compile(r'^sequenceDiagram', re.IGNORECASE),),
        DiagramType.CLASS: (re.compile(r'^classDiagram', re.IGNORECASE),),
        DiagramType.STATE: (re.compile(r'^stateDiagram(-v2)?', re.IGNORECASE),),
        DiagramType.ER: (re.compile(r'^erDiagram', re.IGNORECASE),),
        DiagramType.GANTT: (re.compile(r'^gantt', re.IGNORECASE),),
        DiagramType.PIE: (re.compile(r'^pie', re.IGNORECASE),),
        DiagramType.MINDMAP: (re.compile(r'^mindmap', re.IGNORECASE),),
        DiagramType.TIMELINE: (re.compile(r'^timeline', re.IGNORECASE),),
        DiagramType.QUADRANT: (re.compile(r'^quadrantChart', re.IGNORECASE),),
        DiagramType.REQUIREMENT: (re.compile(r'^requirementDiagram', re.IGNORECASE),),
        DiagramType.JOURNEY: (re.compile(r'^journey', re.IGNORECASE),),
        DiagramType.C4: (
            re.compile(r'^C4Context', re.IGNORECASE),
            re.compile(r'^C4Container', re.IGNORECASE),
            re.compile(r'^C4Component', re.IGNORECASE),
            re.compile(r'^C4Deployment', re.IGNORECASE),
        ),
    })

    def __init__(self, troubleshooting_path: Path | None = None):
        """
    ...
    def _find_troubleshooting_guide(self) -> Path | None:
    ...
    def get_search_recommendation(
        ...
        1. Available documentation or web search tool
        2. Current issue trackers and community discussions
        3. Available independent model or subagent
        4. Another available web search provider
    ...
    def generate(
        ...
        if not matches or (matches and matches[0].get('error_number', 0) == 0):
            search_rec = self.get_search_recommendation(
                error_message or "Unknown rendering error", diagram_type
            )
...
Error Recovery:
  ...
  3. Generates search queries for external troubleshooting:
     - available documentation or web search tool (primary)
     - current issue trackers and community discussions (secondary)
     - available independent model or subagent (tertiary)
     - another available web search provider (fallback)
        """
    )
    ...
    if args.json:
        ...
        if result.success:
            ...
            if result.search_recommendation:
                ...
                print("\n  Troubleshooting sources (in priority order):")
                print("    1. Available documentation or web search tool")
                print("    2. Current issue trackers and community discussions")
                print("    3. Available independent model or subagent")
                print("    4. Another available web search provider")
