...
`cobra.CheckErr(err)` is a convenience function: if `err != nil`, it prints the error to standard error and calls `os.Exit(1)`. Use it only in `main()` where you want a hard exit — not inside `RunE` where returning the error is preferred.
