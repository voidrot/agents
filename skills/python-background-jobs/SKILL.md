...
description: Python background job patterns including task queues, workers, and event-driven
  architecture. Use when implementing async task processing, job queues, long-running
  operations, or decoupling work from request/response cycles.
metadata:
    ...
    async def update_status(
        ...
        if status == JobStatus.RUNNING:
            await self._db.execute(
                ...
                status.value, datetime.utcnow(), job_id,
            )
