...
- Dense + sparse with `prefetch` and fusion [Hybrid search](https://qdrant.tech/documentation/)
- Prefer learned sparse ([miniCOIL](https://qdrant.tech/documentation/), SPLADE, GTE) over raw BM25 if applicable (when user needs smart keywords matching and learned sparse models know the vocabulary of the domain)
- For non-English languages, [configure sparse BM25 parameters accordingly](https://qdrant.tech/documentation/)
- RRF: good default, supports weighted (v1.17+) [RRF](https://qdrant.tech/documentation/)
- DBSF with asymmetric limits (sparse_limit=250, dense_limit=100) can outperform RRF for technical docs [DBSF](https://qdrant.tech/documentation/)
...
- Cross-encoder rerankers via FastEmbed [Rerankers](https://qdrant.tech/documentation/)
- See how to use [Multistage queries](https://qdrant.tech/documentation/) in Qdrant
- ColBERT and ColPali/ColQwen reranking is especially precise due to late interaction mechanisms, but it is heavy. It is important to configure and store multivectors without building HNSW for them to save resources. See [Multivector representation](https://qdrant.tech/documentation/)
...
- RF Query is currently based on a [3-parameter naive formula](https://qdrant.tech/documentation/) with no universal defaults, so it must be tuned per dataset, retriever, and feedback model
...
- Check the configuration of the [Relevance Feedback Query API](https://qdrant.tech/documentation/)
- Use this as a helper end-to-end text retrieval example with parameter tuning and evals to understand how to use the API and run the `qdrant-relevance-feedback` framework: [RF tutorial](https://qdrant.tech/documentation/)
...
- Use MMR (v1.15+) as a query parameter with `diversity` to balance relevance and diversity [MMR](https://qdrant.tech/documentation/)
...
- Recommendation API: positive/negative examples to recommend fitting vectors [Recommendation API](https://qdrant.tech/documentation/)
  - Best score strategy: better for diverse examples, supports negative-only [Best score](https://qdrant.tech/documentation/)
- Discovery API: context pairs (positive/negative) to constrain search regions without a request target [Discovery](https://qdrant.tech/documentation/)
...
Check how to set up in [Score Boosting docs](https://qdrant.tech/documentation/)
