...
### 3. Independent Model or Subagent (Tertiary)

Best for: Complex multi-line errors, alternative perspectives, debugging assistance. Use any independent model or subagent available in the current runtime.
...
3. **Generate code:**
   ...
   ```
4. **Save files:**
   - `./diagrams/auth_doc_01_sequence_user_login.mmd` (generated output)
   - `./diagrams/auth_doc_01_sequence_user_login.png` (generated output)
...
5. **If still failing:** Use another available search provider, then ask an independent model or subagent
