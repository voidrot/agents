---
name: sentry-go-sdk
description: Full Sentry SDK setup for Go. Use when asked to "add Sentry to Go", "install
  sentry-go", "setup Sentry in Go", or configure error monitoring, tracing, logging,
  metrics, or crons for Go applications. Supports net/http, Gin, Echo, Fiber, FastHTTP,
  Iris, Negroni, and gRPC.
license: Apache-2.0
metadata:
  category: sdk-setup
  parent: sentry-sdk-setup
---
> All Skills > sentry-sdk-setup > Go SDK
...
> The minimum Go version follows the selected SDK release (for example, v0.43.0 requires Go 1.24; newer releases may require newer Go versions).
> Always verify the selected version against [docs.sentry.io/platforms/go/](https://docs.sentry.io/platforms/go/) before implementing.
...
# the sentry-go/grpc module exists only from sentry-go v0.46.0+):
...
The gRPC integration module exists only from sentry-go v0.46.0+ (it is absent
at v0.43.0, the reference version above):

```go
import sentrygrpc "github.com/getsentry/sentry-go/grpc"

// Server: register interceptors when creating the gRPC server
// (server interceptors take a required ServerOptions argument)
server := grpc.NewServer(
    grpc.UnaryInterceptor(sentrygrpc.UnaryServerInterceptor(sentrygrpc.ServerOptions{Repanic: true})),
    grpc.StreamInterceptor(sentrygrpc.StreamServerInterceptor(sentrygrpc.ServerOptions{Repanic: true})),
)
...
| Logging | `references/logging.md` | logrus / zap / zerolog / slog detected |
| Metrics | `references/metrics.md` | Business KPIs / SLO tracking |
| Crons | `references/crons.md` | Scheduler / cron job patterns detected |
