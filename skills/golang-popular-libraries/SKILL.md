...
description: Recommends production-ready Golang libraries and frameworks. Apply when
  the user explicitly asks for library suggestions, wants to compare alternatives,
  needs to choose a library for a specific task, or when a new dependency is being
...
- Use `golang-dependency-injection` for samber/do conceptual guidance; verify APIs against current samber/do documentation
- Consult current samber/hot documentation for in-memory caching details
- Consult current samber/oops documentation and `golang-error-handling` for error-handling guidance
