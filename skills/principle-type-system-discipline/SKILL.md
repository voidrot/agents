...
- **External data is untyped until parsed.** RPC payloads, JSON, IPC messages, CLI args, config files, environment variables, database rows. Put a parse function at each boundary that receives unstructured input and turn it into the typed model there.
