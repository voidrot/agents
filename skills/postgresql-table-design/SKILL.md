...
description: Use this skill when designing or reviewing a PostgreSQL-specific schema.
  Covers best-practices, data types, indexing, constraints, performance patterns,
...
- **Limitations**: no global UNIQUE constraints independent of the partition key—include the partition key in PK/UNIQUE constraints. Foreign keys can reference partitioned tables and can be defined on partitioned tables in current PostgreSQL; verify feature-specific limitations when targeting older releases.
...
- **Requires UNIQUE index** on conflict target columns—`ON CONFLICT (col1, col2)` needs a matching unique index (a partial unique index works only when its `WHERE` predicate is given in the conflict target).
