...
- Increase segment count to match CPU cores (`default_segment_number: 16`) [Minimizing latency](https://qdrant.tech/documentation/)
- Keep quantized vectors and HNSW in RAM (`always_ram=true`)
- Reduce `hnsw_ef` at query time (trade recall for speed) [Search params](https://qdrant.tech/documentation/)
...
- Use quantization: scalar (4x reduction) or binary (16x reduction) [Quantization](https://qdrant.tech/documentation/)
- Move payload indexes to disk if filtering is infrequent [On-disk payload index](https://qdrant.tech/documentation/)
...
More RAM and faster CPU directly reduce latency. See [Vertical Scaling](scaling-data-volume/vertical-scaling.md) for node sizing guidelines.
