...
8. **Write Go-focused evidence.** Place `_test.go` files with the package they exercise. Test the observable contract with deterministic inputs and cleanup through `t.Cleanup`; use a same-package test only when access to unexported behavior is necessary, otherwise favor the public API. Use table-driven subtests when several cases share one action and assertion shape, not as ceremony. For I/O, time, randomness, processes, or external services, inject a narrow dependency or controlled input rather than sleeping, relying on order, or contacting production. For changed concurrent code, test cancellation and completion where material, and run the race detector for the affected packages when the environment permits.

9. **Measure before performance changes.** Define the user-relevant metric and a representative workload. Add or run a focused benchmark only when performance is requested or a measured bottleneck warrants it; prevent compiler elimination, reset setup time as needed, and report allocations when they matter. Capture a baseline, inspect profiles when benchmark results do not identify the cause, change one factor, then rerun the same benchmark under comparable conditions. Keep an optimization only when it improves the stated metric without weakening correctness, readability, cancellation, or error behavior. Do not add pooling, unsafe code, concurrency, caching, or tuning based on intuition alone.
