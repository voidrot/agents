...
struct Pool {
    semaphore: Semaphore,
    // Returning a connection is synchronous and does not await while holding
    // this short-lived lock, so Drop can put it back without spawning a task.
...
impl Pool {
    fn new(size: usize) -> Self {
        Self {
            ...
            connections: StdMutex::new((0..size).map(|_| Connection::new()).collect()),
        }
    }
