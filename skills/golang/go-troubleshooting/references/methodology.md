...
# Run linters — configure golangci-lint according to project policy
golangci-lint run ./...
```

Run `golangci-lint` early in your debugging workflow. It catches unchecked errors, suspicious constructs, and many other issues that are easy to miss by reading code. This repo does not currently include a separate local lint skill, so use project policy or golangci-lint documentation for configuration.
