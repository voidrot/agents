...
# Parse from a file
cat bench.txt | gobenchdata --json bench.json

# Add a tag to the benchmark run (e.g., git commit)
cat bench.txt | gobenchdata --json bench.json --tag "$(git rev-parse --short HEAD)"

# Convert baseline and current benchmark output to gobenchdata JSON
# (checks eval compares two JSON benchmark histories.)
cat base.txt | gobenchdata --json base.json
cat current.txt | gobenchdata --json current.json

# Evaluate regression checks against a checks config
gobenchdata checks eval base.json current.json --checks.config .gobenchdata-checks.yml

# Generate the web dashboard app (static Vue.js site)
gobenchdata web generate ./dashboard-app

# Serve the dashboard locally for preview (serves ./benchmarks.json on localhost:8080)
gobenchdata web serve

# Merge multiple benchmark JSON files
gobenchdata merge old-bench.json new-bench.json > combined.json

# Keep the latest 30 runs when appending a new benchmark result
cat bench.txt | gobenchdata --append --prune 30 --json bench.json
```
    ...
    steps:
      ...
      - uses: bobheadxi/gobenchdata@v1
        with:
          PRUNE_COUNT: 30
          GO_TEST_PKGS: ./...
          GO_TEST_FLAGS: -count=5
          BENCHMARKS_OUT: benchmarks.json
          PUBLISH: true
...
```yaml
- name: Convert baseline benchmarks
  run: cat base.txt | gobenchdata --json base.json

- name: Convert current benchmarks
  run: cat current.txt | gobenchdata --json current.json

- name: Check for regressions
  run: gobenchdata checks eval base.json current.json --checks.config .gobenchdata-checks.yml
```
...
checks:
  - name: "No major regressions"
    ...
    thresholds:
      max: 20 # fail if more than 20% slower
  - name: "Critical path stability"
    package: "pkg/parser"
    benchmarks: ["BenchmarkParse.*"]
    diff: "(current.NsPerOp - base.NsPerOp) / base.NsPerOp * 100"
    thresholds:
      max: 10 # stricter: fail if more than 10% slower
```
