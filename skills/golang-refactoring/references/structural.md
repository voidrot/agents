...
- **Caveat: this is a heuristic, not dogma.**
  ...
  - → See `golang-project-layout` skill for the directory/package layout conventions this section assumes, and `golang-design-patterns` skill for judging when introducing an interface is the right call versus premature abstraction.
...
- gopls does not currently expose a package-splitting code action. Group top-level declarations by responsibility and dependency direction manually, then review the resulting API and import graph. For a lighter-weight split that keeps one package, use its `refactor.extract.toNewFile` code action to move declarations between files.
...
- The refactor is toward explicit construction: a constructor function that returns a struct, with dependencies passed in as parameters rather than reached for through a package-level variable or `init()`-populated singleton.
  ...
  - → See `golang-design-patterns` skill for constructor and dependency-injection patterns, and [safety-net.md](safety-net.md) in this skill for how this same seam is what a Feathers-style characterization test exploits to get coverage before a risky change.
...
- → See `golang-project-layout` skill for directory/package layout conventions.
- → See `golang-design-patterns` skill for when an interface is the right design choice versus premature abstraction, and for constructor/DI patterns.
