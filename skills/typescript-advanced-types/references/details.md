...
class APIClient<
  Config extends {
    [Path in keyof Config]: Partial<Record<HTTPMethod, Endpoint>>;
  }
> {
  async request<Path extends keyof Config, Method extends keyof Config[Path]>(
    ...
    // The transport implementation is application-specific.
    throw new Error("request implementation omitted");
  }
}
...
class Builder<T, S extends BuilderState<T> = {}> {
  ...
  set<K extends keyof T>(key: K, value: T[K]): Builder<T, S & Record<K, T[K]>> {
    (this.state as BuilderState<T>)[key] = value;
