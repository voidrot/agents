...
description: Upgrade the Sentry JavaScript SDK across major versions. Use when asked
  to upgrade Sentry, migrate to a newer version, fix deprecated Sentry APIs, or resolve
  breaking changes after a Sentry version bump.
license: Apache-2.0
metadata:
  category: workflow
...
If the user didn't specify a target version, recommend the latest major version (v10 as of this writing). If the user has already bumped package versions but has broken code, detect the target from `package.json`.
