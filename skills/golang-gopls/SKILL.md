---
name: golang-gopls
description: Golang semantic code intelligence via `gopls`, the official Go language
  server — go-to-definition, find references, call/implementation hierarchy, workspace
  symbol search, package API discovery, diagnostics, safe rename, refactors (extract/inline/fill/rewrite
  code actions), formatting, and generated tests. Reaches an agent via gopls's own
  MCP server (`go_*` tools), an available native LSP integration, or the `gopls` CLI.
  Use when navigating or refactoring Go code — jumping to a definition, finding call
  sites before a rename, understanding a file's or package's dependencies, running
  diagnostics after an edit, or extracting/inlining/renaming. Not for the published
  ecosystem — packages not in your `go.mod`, versions, licenses, importers — → See
  `golang-pkg-go-dev` skill (`godig`). Not for a whole-tree
  vulnerability audit → See `golang-security` skill (`govulncheck`).
license: MIT
metadata:
  author: samber
  version: 1.1.0
...
**Dependencies:** `gopls` — `go install golang.org/x/tools/gopls@latest` (v0.20+). An LSP integration also requires a host that supports it (see [references/mcp.md](references/mcp.md)).
...
- **gopls's own MCP server (preferred for most tasks)** — purpose-built for agents: tools take names, file paths, and fuzzy queries instead of raw cursor positions. Configure an MCP-capable host to run `gopls mcp`. It runs headless over stdio, sees files saved to disk, and is a good default for agent-only workflows. See [references/mcp.md](references/mcp.md) for every tool.
- **A native LSP integration** — when supported by the current host, configure it to use `gopls`. Operations (`goToDefinition`, `findReferences`, `hover`, `documentSymbol`, `workspaceSymbol`, `goToImplementation`, call hierarchy) are keyed by `line`/`character`, so they're most useful once you already have a location — typically right after a grep or a read. Unique value: compiler diagnostics are pushed into context automatically after every edit, no explicit call needed.
...
Choose by evidence source: use `gopls` for the resolved workspace, `golang-pkg-go-dev` for the published ecosystem, Context7 for current external documentation, and `golang-security` for whole-tree vulnerability analysis.
