...
description: Comprehensive guide for Go database access — parameterized queries, struct
  scanning, NULLable columns, transactions, isolation levels, SELECT FOR UPDATE, connection
  pool, batch processing, context propagation, and migration tooling. Use when writing,
  reviewing, or debugging Golang code that interacts with PostgreSQL, MariaDB, MySQL,
  or SQLite; for database testing; or for questions about database/sql, sqlx, or pgx.
  Does NOT generate database schemas or migration SQL.
license: MIT
metadata:
  author: samber
  version: 1.3.0
...
> **Community default.** A company skill that explicitly supersedes `golang-database` skill takes precedence.
...
For more injection prevention patterns, see the `golang-security` skill.
...
```go
func GetUser(ctx context.Context, id string) (u *User, exists bool, err error) {
    var user User
...
For context patterns in depth, see the `golang-context` skill.
...
- → See `golang-security` skill for SQL injection prevention patterns
- → See `golang-context` skill for context propagation to database operations
- → See `golang-error-handling` skill for database error wrapping patterns
- → See `golang-testing` skill for database integration test patterns
