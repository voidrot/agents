...
Migrated from `skills/.to-sort/react19-source-patterns/SKILL.md` into `skills/typescript/react-modernization`.
...
| Legacy Context | → `createContext` | [→ api-migrations.md#legacy-context](react19-source-patterns-api-migrations.md#legacy-context) |
| String refs `this.refs.x` | → `createRef()` | [→ api-migrations.md#string-refs](react19-source-patterns-api-migrations.md#string-refs) |
...
For full before/after code for each migration, read **`references/react19-source-patterns-api-migrations.md`**. It contains the complete patterns including edge cases for `forwardRef` with `useImperativeHandle`, `defaultProps` null vs undefined behavior, and legacy context provider/consumer cross-file migrations.
